...
public class FrontendFrame extends JFrame
{
    ...
    // Constants
    public static final ListCellRenderer LIST_RENDERER = new ModifiedDefaultListCellRenderer();
    public static final Color buttonsHColor = new Color(255, 255, 255);
        ...
        try
        {
            ...
            if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    if (el.getContent().equalsIgnoreCase(fpath)) found = true;
                }
                if (found) return;
    ...
    private void bookmarkLoad(String file)
    {
        ...
        {
            // We get the base path
            int index = -1, size = Frontend.bookmarks.size();
            for (int i = 0; i < size; i++)
            {
                XMLElement e = (XMLElement) Frontend.bookmarks.get(i);
                if (e.getContent().equalsIgnoreCase(file)) index = i;
            }
            if (index == -1) return;
