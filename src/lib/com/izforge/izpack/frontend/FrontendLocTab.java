...
public class FrontendLocTab extends FrontendTab implements ActionListener
{
  ...
  /**  The current available pack. */
  private int curAv = -1;

  /**  The current selected pack. */
  private int curSel = -1;

  /**  The langpacks directory. */
   ...
   *  The constructor.
   *
   * @param  installation  The XML installation tree.
  ...
  private void initLists()
  {
    try
    {
      ...
      for (int i = 0; i < size; i++)
        avPacks.add(packs[i].substring(0, 3));
      selPacks = new Vector();
    }
    catch (Exception err)
    {
