 ...
 *  registry and load save and manipulate link data. The native code is
 *  contained in the file <code>ShellLink.cpp</code>. <br>
 *  <br>
 ...
 *  <br>
 *  <A HREF=http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dnmgmt/html/msdn_shellnk1.asp>
 *  Using Shell Links in Windows 95</A> <br>
 *  <A HREF=http://msdn.microsoft.com/library/default.asp?url=/library/en-us/dnmgmt/html/msdn_shellnk2.asp>
 *  The IShellLink Interface</A> <br>
 *  <A HREF=http://msdn.microsoft.com/library/default.asp?url=/library/en-us/shellcc/platform/Shell/IFaces/IShellLink/IShellLink.asp>
 *  IShellLink</A>
 *
 * @author     Elmar Grom
...
public class ShellLink implements NativeLibraryClient
{
   ...
   */
  public final static int MINIMIZED = 2;
  /**  Show the window maximized when starting. */
  public final static int MAXIMIZED = 3;

  private final static int MIN_SHOW = 0;
  private final static int MAX_SHOW = 3;
   ...
   */
  private final static int SL_WRONG_DATA_TYPE = -7;
  ...
  private final static String LINK_EXTENSION = ".lnk";
  /**  the constant to use for selecting the current user. */
  public final static int CURRENT_USER = 0;
   ...
   *  variable contains only valid data if the link was created from a disk file
   *  or after a successful save operation. At other times the content is
   *  upredicatable.
   */
   ...
   *  variable. This caused problems in <code>set()</code> For this reason, the
   *  dummy string is placed here. Observed with version: <pre>
   *java version "1.3.0"
   ...
   *      </ul>
   *
   * @param  name                          The name that the link should display
   *      on a menu or on the desktop. Do not include a file extension.
   * @exception  IllegalArgumentException  if any of the call parameters are
   *      incorrect
   * @exception  Exception                 if problems are encountered in
   *      initializing the native interface
   */
  /*
   *  --------------------------------------------------------------------------
   */
   ...
   *  on disk.
   *
   * @param  name                          the fully qualified file name of the
   *      link.
   ...
   *  on disk.
   *
   * @param  type                          The type of link, one of the
   *      following values: <br>
   *
   *      <ul>
   *        <li> <code>DESKTOP</code>
   ...
   *
   * @param  userType                      the type of user for the link path.
   * @param  group                         The program group this link is a part
   *      of. If the link is not part of a program group, pass an empty string
   *      or null for this parameter.
   * @param  name                          The file name of this link. Do not
   ...
   *  deleting it from its temporary loaction. A call to this method will freeze
   *  the application irrecoverably!
   *
   ...
   *
   * @param  hotkey  a valid Windows virtual key code. Modifiers (e.g. for alt
   *      or shift key) are added in the upper byte. Note that only the lower 16
   *      bits for tis parameter are used.
   * @see            #getHotkey
   */
  /*
   *  --------------------------------------------------------------------------
   ...
   *
   * @param  name                          the fully qualified file name for the
   *      link
   * @exception  Exception                 if the save operation could not be
   *      carried out
   */
  /*
   *  --------------------------------------------------------------------------
