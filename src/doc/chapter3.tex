...
When building a web installer you get two files at the installer
compilation time. The one to put on the server ends with '\texttt{\_web}'.
The location of this file must be in a resource named
\texttt{WebInstallers.url}. Just make a text file containing on the
first line the URL to the file. And that's it.\\
