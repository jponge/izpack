...
so you might rather want to work with a text editor havaing such a
...
\begin{itemize}
  ...
  \item Variables that you can define. You also define the value,
        wich is fixed for a given installation file.
\end{itemize}

You define your own variables in the installation XML file with the
texttt{<variable>} tag. How to do this is explained in detail later in
this cahpter.\\

textbf\{Please note} that when using variables they must always appear
...
\texttt{\$INSTALL\_PATH} (which is the installation path choosen by the
...
to parse. The variable subsitutor will then replace it with the correct
